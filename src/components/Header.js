...
const Header = () => {
  ...
  const { username, uri } = useSelector((state) => state.loggedInUser);
        ...
        <Nav className="header-right-nav">
          {!username && (
            <ButtonToolbar>
              ...
              <Button as={Link} to="/signup" variant="primary" className="ml-2">
                Sign Up
              </Button>
            </ButtonToolbar>
          )}
          {username && (
            <Nav>
              <Nav.Link as={Link} to="/playlists/create" variant="pills">
                Create Playlist
              </Nav.Link>
              <Nav.Link as={Link} to="/upload" variant="pills">
                Upload
              </Nav.Link>
              <NavDropdown title={username} className="header-right-dropdown">
                <NavDropdown.Item as={Link} to={`/users/${uri}`}>
                  My Profile
                </NavDropdown.Item>
